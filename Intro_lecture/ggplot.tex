...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
p <- \hlkwd{ggplot}(data = iris, \hlkwd{aes}(x = Sepal.Length, y = Sepal.Width))
\hlkwd{summary}(p)
\end{alltt}
\begin{verbatim}
...
\begin{column}[T]{8cm}
\begin{itemize}
    \item \textbf{The geometric objects on the plot}
...
\begin{column}[T]{8cm}
\begin{itemize}
    \item Not going to cover these in detail
    \item for e.g. polar coordinate plots
    \item or using a custom model to transform data
\end{itemize}
\end{column}
